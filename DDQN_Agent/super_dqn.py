...
DEFAULT_CONFIG = {
    ...
    'beta_increment_prioritized': 0.001,
    'epsilon_prioritized': 1e-6,
    ...
    def __init__(self, input_size, out_rate_size, out_time_size, configs):
        ...
        if self.configs.get('prioritized_replay', False):
            self.experience_replay = PrioritizedReplayMemory(
                ...
                alpha=self.configs.get('alpha_prioritized', 0.6),
                beta=self.configs.get('beta_prioritized', 0.4),
                beta_increment_per_sampling=self.configs.get('beta_increment_prioritized', 0.001)
            )
        else:
...
class Trainer(RLAlgorithm):
            ...
            for index in torch.nonzero(mask):
                if self.configs['mode'] == 'train':
                    if random.random() > self.epsilon:  # epsilon greedy
                        ...
                        # 如果agent增加则使用view(agents,action_size)
                    else:
    ...
    def update(self, mask):  # 对每个agent执行 # 回到agent网络执行应该可以
        ...
        if len(self.mainSuperQNetwork.experience_replay) > self.configs['batch_size'] and mask.sum() > 0:
            prioritized_replay = self.configs.get('prioritized_replay', False)
            ...
            # 计算Q(s_t, a) - 当模型计算action batch的a'时的Q(s_t,a')，选择采取的行动a'的列(列是Q值)
            ...
            #     self.device)  # .to(self.configs['device'])  # 从自己的Q值中获取最大的值
            ...
            time_next_state_values[non_final_mask] = time_Q.detach().gather(dim=1,index=behavior_time_action).view(-1)  # .to(self.configs['device'])  # 从自己的Q值中获取最大的值

            # 计算期望Q值
            reward_scale = self.configs.get('reward_scaling', 1.0)
            rate_expected_state_action_values = (
                rate_next_state_values * self.configs['gamma']) + (reward_batch * reward_scale)
            time_expected_state_action_values = (
                time_next_state_values * self.configs['gamma']) + (reward_batch * reward_scale)
            ...
            # Apply weights if using prioritized replay
            if prioritized_replay:
                rate_loss = (rate_loss * weights.unsqueeze(1)).mean()
                time_loss = (time_loss * weights.unsqueeze(1)).mean()
            ...
            # retain_graph的原因是为了不影响mainSuperQ
            rate_loss.backward(retain_graph=True)
            time_loss.backward()
            # total_loss.backward(retain_graph=True)
            for param in self.mainSuperQNetwork.parameters():
                param.grad.data.clamp_(-1, 1)  # 将值限制在-1和1之间 (clipping)
            self.optimizer.step()
            
            # Update priorities if using prioritized replay
            if prioritized_replay:
                rate_delta = torch.abs(rate_state_action_values.squeeze() - rate_expected_state_action_values).detach().cpu().numpy()
                time_delta = torch.abs(time_state_action_values.squeeze() - time_expected_state_action_values).detach().cpu().numpy()
                delta = (rate_delta + time_delta) / 2.0
                priorities = delta + self.configs.get('epsilon_prioritized', 1e-6)
                self.mainSuperQNetwork.experience_replay.update_priorities(indices, priorities)
    ...
    def update_tensorboard(self, writer, epoch):
        writer.add_scalar('episode/loss', self.running_loss/self.configs['max_steps'],
                          self.configs['max_steps']*epoch)  # 每1个epoch
